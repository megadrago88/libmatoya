...
static bool test_rw_locks()
{
	...
	test_cmp("MTY_RWLock read counter", data.counter == 0);
	MTY_Sleep(100);
	MTY_WaitableSignalAll(data.wait);
	MTY_Sleep(10);
	test_cmp("MTY_RWLock read counter", data.counter == 0);
...
static bool test_waitables()
{
	...
	MTY_Sleep(100);
	test_cmp("MTY_Atomic32Get", MTY_Atomic32Get(&data.atomic_32) == 1);
	MTY_Sleep(100);
	MTY_WaitableSignalAll(data.wait);
	...
	MTY_WaitableSignal(data.wait);
	MTY_Sleep(100);
	test_cmp("MTY_Atomic64Get", MTY_Atomic64Get(&data.atomic_64) == 1);
	MTY_Sleep(100);
	MTY_WaitableSignalAll(data.wait);
	...
	test_cmp("MTY_Atomic64Get", MTY_Atomic64Get(&data.atomic_64) == test_thread_count);
...
static bool test_conditions() {
	...
	for (int32_t i = 0; i < test_thread_count; i++) {
		...
		if (data.counter != i) {
			test_cmp("MTY_Mutex t counter", data.counter == i);
		}
	}
