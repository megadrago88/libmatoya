...
static void mty_rwlock_create(mty_rwlock *rwlock)
{
	...
	if (e != 0)
		MTY_Fatal("'pthread_rwlock_init' failed with error %d", e);

	e = pthread_rwlockattr_destroy(&attr);
	if (e != 0)
		MTY_Fatal("'pthread_rwlockattr_destroy' failed with error %d", e);
}
...
static void mty_rwlock_writer(mty_rwlock *rwlock)
{
	...
	if (e != 0)
		MTY_Fatal("'pthread_rwlock_wrlock' failed with error %d", e);
}
...
static void mty_rwlock_destroy(mty_rwlock *rwlock)
{
	...
	if (e != 0)
		MTY_Fatal("'pthread_rwlock_destroy' failed with error %d", e);
}
