...
struct gfx_ui *gfx_metal_ui_create(MTY_Device *device)
{
	...
	if (!r)
		gfx_metal_ui_destroy((struct gfx_ui **) &ctx);

	return (struct gfx_ui *) ctx;
}

bool gfx_metal_ui_render(struct gfx_ui *gfx_ui, MTY_Device *device, MTY_Context *context,
	const MTY_DrawData *dd, MTY_Hash *cache, MTY_Texture *dest)
{
	struct gfx_metal_ui *ctx = (struct gfx_metal_ui *) gfx_ui;
	id<MTLDevice> _device = (__bridge id<MTLDevice>) device;
...
void *gfx_metal_ui_create_texture(MTY_Device *device, const void *rgba, uint32_t width, uint32_t height)
{
	id<MTLDevice> _device = (__bridge id<MTLDevice>) device;
...
void gfx_metal_ui_destroy_texture(void **texture)
{
	if (!texture || !*texture)
		return;
...
void gfx_metal_ui_destroy(struct gfx_ui **gfx_ui)
{
	if (!gfx_ui || !*gfx_ui)
		return;
