...
struct tls *tls_connect(TCP_SOCKET socket, const char *host, uint32_t timeout)
{
	...
	MTY_Async a = tls_engine_handshake(ctx->engine, NULL, 0, tls_write_callback, ctx);
	...
	while (true) {
		...
		a = tls_engine_handshake(ctx->engine, ctx->buf, size, tls_write_callback, ctx);
		if (a == MTY_ASYNC_OK) {
			break;
...
bool tls_write(struct tls *ctx, const void *buf, size_t size)
{
	...
	// Encrypt
	size_t written = 0;
	if (!tls_engine_encrypt(ctx->engine, buf, size, ctx->buf, ctx->buf_size, &written))
		return false;
...
bool tls_read(struct tls *ctx, void *buf, size_t size, uint32_t timeout)
{
	while (true) {
		...
		// Decrypt
		size_t read = 0;
		if (!tls_engine_decrypt(ctx->engine, ctx->buf, msg_size, ctx->buf, ctx->buf_size, &read))
			break;
