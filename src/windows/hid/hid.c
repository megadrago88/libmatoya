...
static struct hid *hid_device_create(HANDLE device)
{
	struct hdevice *ctx = MTY_Alloc(1, sizeof(struct hid));
...
void hid_win32_feed_msg(struct hid *ctx, uint32_t msg, uintptr_t lparam, uintptr_t wparam)
{
}
